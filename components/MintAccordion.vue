	...
	<div class="accordion" :class="{ active: isActive }">
		<LayoutFlex direction="row-start-space-between">
			<LayoutFlex
				...
				<img v-if="selected.icon" :src="require(`~/assets/images/borrow/${selected.icon}`)" :alt="`${selected.symbol} logo`">
			...
			<div class="accordion__tokens">
				<div
					...
					<div class="token__wrapper">
						<img :src="require(`~/assets/images/borrow/${token.icon}`)" :alt="`${token.name} logo`">
...
import { mintTokens } from "~/constants/tokens";
